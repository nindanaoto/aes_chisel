...
// implements SubWord
class SubWord() extends Module {
  val io = IO(new Bundle {
    val word_in = Input(Vec(Params.Nb, UInt(8.W)))
    val word_out = Output(Vec(Params.Nb, UInt(8.W)))
  })
  ...
  for (i <- 0 until Params.Nb) {
    io.word_out(i) := s_box(io.word_in(i))
  }
}

class KeyExpansion(Nk: Int) extends Module {
	require(Nk == 4 || Nk == 6 || Nk == 8)
	val Nr = Nk + 6
	val io = IO(new Bundle {
	val key = Input(Vec(Nk, Vec(Params.StateLength, UInt(8.W))))
	val roundKeys = Output(Vec(Nr+1,Vec(Params.StateLength, UInt(8.W))))
	})
	...
	val rcon = VecInit(Array(  
		...
		0x40.U, /* x^6 */
		0x80.U, /* x^7 */
		0x1B.U, /* x^4 + x^3 + x^1 + x^0 */
		0x36.U, /* x^5 + x^4 + x^2 + x^1 */
		))
	val w = Wire(Vec(Params.Nb*(Nr+1),Vec(Params.rows, UInt(8.W))))
	
	for(i <- 0 until Nk ){
		for(j <- 0 until Params.rows){
			val index = i*Params.rows + j
			w(i)(j) := io.key(index/Params.StateLength)(index%Params.StateLength)
		}
	}
	for(i <- Nk until Params.Nb*(Nr+1)){
		val temp = Wire(Vec(Params.rows, UInt(8.W)))
		temp := w(i-1)
		if(i%Nk==0){
			val subword = Module(new SubWord())
			// RotWord xor Rcon[i/Nk]
			for(j <- 0  until Params.rows){
				subword.io.word_in(j) := w(i-1)((j+1)%Params.rows)
			}
			temp := subword.io.word_out
			temp(0) := subword.io.word_out(0) ^ rcon(i/Nk)
		}else if (6 < Nk && i % Nk == 4){
			val subword = Module(new SubWord())
			subword.io.word_in := w(i-1)
			temp := subword.io.word_out
		}
		for(j <- 0  until Params.rows){
	...
	for(i <- 0 until Nr+1){
		for(j <- 0 until Params.StateLength){
			...
			io.roundKeys(i)(j) := w(index/Params.rows)(index%Params.rows)
		}
	}
}